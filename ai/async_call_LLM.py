...
async def main():
    ...
    tasks = [translator(d, target_language) for d in dicts.values()]
    translated_dicts = await asyncio.gather(*tasks)
    
    for i, key in enumerate(dicts.keys()):
        print(f"Original dictionary {key}:\n{json.dumps(dicts[key], ensure_ascii=False, indent=2)}\n")
        print(f"Translated to {target_language}:\n{json.dumps(translated_dicts[i], ensure_ascii=False, indent=2)}\n")
